...
{
    //更改订单状态
    class SynchronizeOrderCFStatus
    {
        ...
        public string startWork()
        {
            ...
            try
            {//1、回传订单状态
                if (ComonUtil.g_IsStaFinishCF)
                {
                    ComonUtil.g_IsStaFinishCF = false;

                    ValObj vo = new ValObj();
                    vo.rowNum = Convert.ToInt32(ComonUtil.packSize);
                    vo.orderFrom = "2";
                    var sls = staUp.getBillState(vo);
                    if (sls == null || sls.Count == 0) { ComonUtil.g_IsStaFinishCF = true;  }
                    else{
                        SerResponse serRe = ObjConvertUtil.Json2Object<SerResponse>
                            (re = DoPost(ComonUtil.serviceCFUrl, "upBillState", ObjConvertUtil.Object2Json(sls)));
    ...
    #endif
                        ComonUtil.g_IsStaFinishCF = true;
                    }
                }
                else
                {
            ...
            try
            {//2、回传订单
                if (ComonUtil.g_IsOBillFinishCF)
                {
                    ComonUtil.g_IsOBillFinishCF = false;
                    var sls = getOutBill();
                    if (sls == null || sls.Count == 0) { ComonUtil.g_IsOBillFinishCF = true; }
                    else{
                        var aa = ObjConvertUtil.Object2Json(sls);
                        SerResponse serRe = ObjConvertUtil.Json2Object<SerResponse>
                            (re = DoPost(ComonUtil.serviceCFUrl, "upOutBill", ObjConvertUtil.Object2Json(sls)));
                        ...
                        Logger.WritingLog((new StringBuilder()).Append(re).Append("\r\n").ToString());
    #endif
                        ComonUtil.g_IsOBillFinishCF = true;
                    }
                }
                else
                {
            ...
            try 
            {//3、回传退货单
                if (ComonUtil.g_IsRefundBillFinishCF)
                {
                    ComonUtil.g_IsRefundBillFinishCF = false;
                    staUp.updateToThree();
                    var sls = getRefundBill();
                    if (sls == null || sls.Count == 0) { ComonUtil.g_IsRefundBillFinishCF = true; }
                    else{
                        var aa = ObjConvertUtil.Object2Json(sls);
                        SerResponse serRe = ObjConvertUtil.Json2Object<SerResponse>
                            (re = DoPost(ComonUtil.serviceCFUrl, "upRetBill", ObjConvertUtil.Object2Json(sls)));
                        ...
                        Logger.WritingLog((new StringBuilder()).Append(re).Append("\r\n").ToString());
    #endif
                        ComonUtil.g_IsRefundBillFinishCF = true;
                    }
                }
                else
                {
