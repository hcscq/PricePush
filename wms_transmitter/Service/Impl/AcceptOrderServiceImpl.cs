...
{
    //订单接收接口
    class AcceptOrderServiceImpl : IOrderHandle
    {
        /// <summary>
        /// 解析出报文对象
        /// </summary>
        ...
        public void doHandle(string objson, Tesla.Net.SimpleResponse response)
        {
            ...
            Logger.WritingLog("订单接收=>订单号：" + bm.djbh);//记录接收日志
            ...
            try
            {
                ...
                Logger.WritingLog("==>校验物流商是否存在开始。。。订单号：" + bm.djbh);
                ...
                Logger.WritingLog("==>校验运单号开始。。。订单号：" + bm.djbh);
                if (bm.callstatus == "DZ")
                {
                    if (bm.waybill == null || bm.waybill == "")
                    {
                        msg = "订单" + bm.djbh + "运单号不能为空！";
                        Logger.WritingLog(msg);
                        throw new InterfaceException(InterfaceExceptionType.NoReTry, msg);
                    }
                }
                Logger.WritingLog("==>校验商品是否存在开始。。。订单号：" + bm.djbh);
                for (int i = 0; i < bm.dList.Count; i++)
                {
                    ...
                    if (spid == "" || spid == null)
                    {
                        msg = "订单" + bm.djbh + "商品" + vo.spbh + "不存在！";
                        Logger.WritingLog(msg);
                        throw new InterfaceException(InterfaceExceptionType.NoReTry, msg);
                    }
                    bm.dList[i].spid = spid;
                }
                Logger.WritingLog("==>插入DB开始。。。订单号：" + bm.djbh);
                IBillingMDao billDao = new BillingMDaoImpl();
                if (billDao.insertBillingMD(bm))
                {
                    Logger.WritingLog("订单" + bm.djbh + "接收成功！");//记录接收日志
                    Logger.WriteLog(bm.djbh, "AcceptOrderService", 10, "订单下传");//记录操作日志
                }
                else
                {
                    Logger.WritingLog("订单" + bm.djbh + "接收失败！报文：\n" + objson);//记录接收日志
