...
{
    //订单锁定接口
    class OrderLockServiceImpl : IOrderHandle
    {
        /// <summary>
        /// 解析出报文对象
        /// </summary>
        ...
        public void doHandle(string objson, Tesla.Net.SimpleResponse response)
        {
            ...
            Logger.WritingLog("单据锁定=>单据号：" + bm.djbh);
            ...
            {
                msg = "单据" + bm.djbh + "已是发货状态不能锁定！";
                Logger.WritingLog(msg);
                throw new InterfaceException(InterfaceExceptionType.NoReTry, msg);
            }
            ITaskMDao dao = new TaskMDaoImpl();
            int r = Dao.updateBillingMLock(bm);
            if (r > 0)
            {
                Logger.WritingLog("单据" + bm.djbh + "锁定成功！");
                Logger.WriteLog(bm.djbh, "SYS", 120, "订单锁定");
            }
            else
            {
                msg = "订单" + bm.djbh + "锁定失败！";
                Logger.WritingLog(msg);//记录锁定日志
                throw new InterfaceException(InterfaceExceptionType.NoReTry, msg);
            }
        }
    }
