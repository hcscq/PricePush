...
{
    //（处方药）销售退货接口
    class ReturnOrderCFServiceImpl : IOrderHandle
    {
        ...
        public void doHandle(string objson, Tesla.Net.SimpleResponse response)
        {
            ...
            {
                m.opform = "2";
                Logger.WritingLog("（处方药）退货单据：" + m.djbh);
                ...
                for (int i = 0; i < m.dList.Count; i++)
                {
                    ...
                    if (spid == null || spid == "") throw new InterfaceException(InterfaceExceptionType.NoReTry, "（处方药）退货单据" + m.djbh + "的商品编号" + vo.spbh + "不存在！"); 
                    m.dList[i].spid = spid;
                }
                INoticeMDao dao = new NoticeMDaoImpl();
                if (dao.insertNoticeMD(m))
                {
                    Logger.WritingLog("（处方药）退货单" + m.djbh + "接收成功！");
                    Logger.WriteLog(m.djbh, "ReturnOrderCFService", 10, "（处方药）退货单下传");//记录操作日志
                }
                else
                {
                    Logger.WritingLog("（处方药）退货单" + m.djbh + "接收失败！");
                }
            }
        }
    }
